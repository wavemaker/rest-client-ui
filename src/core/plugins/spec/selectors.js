...
export const value = createSelector(state, (spec) => spec.get("value"));
...
export const configModal = createSelector(state, (spec) =>
  spec.get("configModal")
);
export const providerConf = createSelector(state, (spec) =>{
  return spec.get("providerConf").toJS();
});

export const objval = createSelector(state, (spec) => {
...
export const specJson = createSelector(state, (spec) =>
  spec.get("json", Map())
);

export const specJS = createSelector(specJson, (spec) => spec.toJS());

export const specResolved = createSelector(state, (spec) =>
  spec.get("resolved", Map())
);
...
export const info = createSelector(spec, (spec) =>
  returnSelfOrNewMap(spec && spec.get("info"))
);
...
export const paths = createSelector(specJsonWithResolvedSubtrees, (spec) =>
  spec.get("paths")
);
...
export const consumes = createSelector(spec, (spec) =>
  Set(spec.get("consumes"))
);

export const produces = createSelector(spec, (spec) =>
  Set(spec.get("produces"))
);

export const security = createSelector(spec, (spec) =>
  spec.get("security", List())
);

export const securityDefinitions = createSelector(spec, (spec) =>
  spec.get("securityDefinitions")
);

export const findDefinition = (state, name) => {
  const resolvedRes = state.getIn(
    ["resolvedSubtrees", "definitions", name],
    null
  );
...
export const schemes = createSelector(spec, (spec) =>
  spec.get("schemes", Map())
);
  ...
  (operations, tags) => {
    return operations.reduce(
      (taggedMap, op) => {
        ...
        return tags.reduce(
          (res, tag) => res.update(tag, List(), (ar) => ar.push(op)),
          taggedMap
        );
      },
  ...
  ({ getConfigs }) => {
    ...
    return operationsWithTags(state)
      .sortBy(
        ...
        (tagA, tagB) => {
          let sortFn =
            typeof tagsSorter === "function"
              ? tagsSorter
              : sorters.tagsSorter[tagsSorter];
          return !sortFn ? null : sortFn(tagA, tagB);
        }
      )
      .map((ops, tag) => {
        let sortFn =
          typeof operationsSorter === "function"
            ? operationsSorter
...
export const requests = createSelector(state, (state) =>
  state.get("requests", Map())
);

export const mutatedRequests = createSelector(state, (state) =>
  state.get("mutatedRequests", Map())
);
...
export const parameterWithMetaByIdentity = (state, pathMethod, param) => {
  const opParams = specJsonWithResolvedSubtrees(state).getIn(
    ["paths", ...pathMethod, "parameters"],
    OrderedMap()
  );
  const metaParams = state.getIn(
    ["meta", "paths", ...pathMethod, "parameters"],
    OrderedMap()
  );

  const mergedParams = opParams.map((currentParam) => {
    const inNameKeyedMeta = metaParams.get(
      `${param.get("in")}.${param.get("name")}`
    );
    const hashKeyedMeta = metaParams.get(
      `${param.get("in")}.${param.get("name")}.hash-${param.hashCode()}`
    );
    return OrderedMap().merge(currentParam, inNameKeyedMeta, hashKeyedMeta);
  });
  return mergedParams.find(
    (curr) =>
      curr.get("in") === param.get("in") &&
      curr.get("name") === param.get("name"),
    OrderedMap()
  );
};

export const parameterInclusionSettingFor = (
  state,
  pathMethod,
  paramName,
  paramIn
...
export const parameterWithMeta = (state, pathMethod, paramName, paramIn) => {
  ...
  const currentParam = opParams.find(
    (param) => param.get("in") === paramIn && param.get("name") === paramName,
    OrderedMap()
  );
...
export const operationWithMeta = (state, path, method) => {
  const op = specJsonWithResolvedSubtrees(state).getIn(
    ["paths", path, method],
    OrderedMap()
  );
  ...
  return OrderedMap().merge(op, meta).set("parameters", mergedParams);
};

// Get the parameter value by parameter name
export function getParameter(state, pathMethod, name, inType) {
  pathMethod = pathMethod || [];
  let params = state.getIn(
    ["meta", "paths", ...pathMethod, "parameters"],
    fromJS([])
  );
  return (
...
export const hasHost = createSelector(spec, (spec) => {
...
export function parameterValues(state, pathMethod, isXml) {
  pathMethod = pathMethod || [];
  let paramValues = operationWithMeta(state, ...pathMethod).get(
    "parameters",
    List()
  );
  return paramValues.reduce((hash, p) => {
    let value =
      isXml && p.get("in") === "body" ? p.get("value_xml") : p.get("value");
...
export function contentTypeValues(state, pathMethod) {
  pathMethod = pathMethod || [];
  let op = specJsonWithResolvedSubtrees(state).getIn(
    ["paths", ...pathMethod],
    fromJS({})
  );
  ...
  const requestContentType = meta.get("consumes_value")
    ? meta.get("consumes_value")
    : parametersIncludeType(parameters, "file")
    ? "multipart/form-data"
    : parametersIncludeType(parameters, "formData")
...
export function currentProducesFor(state, pathMethod) {
  ...
  const operation = specJsonWithResolvedSubtrees(state).getIn(
    ["paths", ...pathMethod],
    null
  );
  ...
  const currentProducesValue = state.getIn(
    ["meta", "paths", ...pathMethod, "produces_value"],
    null
  );
...
export const operationScheme = (state, path, method) => {
  ...
  return (
    state.getIn(["scheme", path, method]) ||
...
export const getOAS3RequiredRequestBodyContentType = (state, pathMethod) => {
  ...
  let requestBody = state.getIn(
    ["resolvedSubtrees", "paths", ...pathMethod, "requestBody"],
    fromJS([])
  );
...
export const isMediaTypeSchemaPropertiesEqual = (
  ...
  let requestBodyContent = state.getIn(
    ["resolvedSubtrees", "paths", ...pathMethod, "requestBody", "content"],
    fromJS([])
  );
  ...
  let currentMediaTypeSchemaProperties = requestBodyContent.getIn(
    [currentMediaType, "schema", "properties"],
    fromJS([])
  );
  let targetMediaTypeSchemaProperties = requestBodyContent.getIn(
    [targetMediaType, "schema", "properties"],
    fromJS([])
  );
  return !!currentMediaTypeSchemaProperties.equals(
    targetMediaTypeSchemaProperties
  );
};
